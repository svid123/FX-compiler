...
{
	if (m_pOwner->GetDep().size()>(size_t)(m_pOwner->m_nLastStopLine>>24 & 0xFF))
	{
...
CPreprocessor::SPragmaScope *CPreprocessor::FindPragmaScope(PREP_PRAGMAS PP,int nTokenNum)
{
	...
	for (n=0;n<(int)aScopes.size() && aScopes[n].nBeginToken<=nTokenNum;++n);
	...
	for (int n=(int)dest.size()-1;n>=0;--n)	
	{
		...
		if (rT.T==ETN_PREP_ID && m_mMacros.find(rT.sText)!=m_mMacros.end())
		{
			...
			if (n<(int)src.size()-1 && src[n+1].T==ETN_PREP_MBRACEC_OPEN)
			{
				...
				for (m=n+2;m<(int)src.size() && nLevel;++m)
				{
					TToken &rT=dest[m];
				...
				if (pMacro->IsVarArgs())
					bArgsMatch=((int)aaTokens.size()>=pMacro->GetArgsCount());
				else
